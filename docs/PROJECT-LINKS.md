...
个人简历站「项目」区包含五个作品，其中 **student_ddl**、**startrail_notes**、**traveler_weather**、**traveler_ai** 通过同域子路径预览。
...
| 旅行者校园天气助手 | `http://127.0.0.1:3003/` |
| 旅伴 AI | `http://127.0.0.1:3004/` |

## 国内线上（EdgeOne Pages：一次构建、一次部署）
...
- `dist/traveler-weather/` — 旅行者天气助手（`/traveler-weather/`）
- `dist/traveler-ai/` — 旅伴 AI（`/traveler-ai/`）

EdgeOne Pages 将 `dist` 部署到 `/` 后，「预览」即可用，**无需单独部署子项目仓库**。

构建命令请设为 **`npm run build:site`**（与 `edgeone.json` 一致），不要只用 `build:root`。

环境变量（`edgeone.json` / Actions 工作流已包含）：
...
若子项目使用**独立域名**，在 EdgeOne 环境变量改为完整 URL，并改回 `npm run build:root` 构建作品集。

## 子项目返回作品集

- **student_ddl** 页头「返回作品集」：`VITE_PORTFOLIO_URL`（默认同域 `/`）
- **startrail_notes** 页头「返回作品集」：同上
- **traveler_weather** 页头「返回作品集」：同上
- **traveler_ai** 侧栏「返回作品集」：同上
...
- `traveler-weather-cover.png`
- `traveler-ai-cover.png`
